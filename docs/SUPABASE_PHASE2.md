...
This repository now has a Supabase Auth client/context, profile service/edit flow, an initial database migration, and local Auth redirect settings. `useAuth()` exposes `loading`, `authenticated`, and `unauthenticated` states; the Supabase client persists and refreshes browser sessions. **The migration has not been applied to a hosted project in this workspace.** No Supabase project URL, publishable key, Google OAuth credentials, or SMTP credentials were supplied, so hosted sign-up, email delivery, OAuth, and Storage access remain unverified.
...
The migration is `supabase/migrations/20260924000000_phase2_xshop_foundation.sql`. Review it and apply it to the intended project, for example by linking the Supabase CLI to that project and running `supabase db push`, or by reviewing/running the SQL through the Supabase SQL Editor. Do not apply it to an unintended production project. This migration creates profiles, role helper/trigger functions, RLS policies, and Storage buckets/policies only; it does not create store catalog, orders, payments, or digital-inventory tables.
